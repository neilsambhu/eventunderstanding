...
def main(annotations_path, results_main_path, for_multiple_segments=False, ranks=[1]):
    annotation_files = [ ]
    recursive_file_search([annotations_path], annotation_files, ['coarse'])
    video_performance_rates = {}
    if for_multiple_segments:
...
if __name__ == '__main__':
    ranks = [1]
    for_multiple_segments = False

    if len(sys.argv) < 3:
        print './measure_performance.py <annotations_path> <results_main_path> OPTIONS'
        print 'OPTIONS:'
        print '-m : evaluating performance for multiple-segment experiments'
    ...
    main(annotations_path, results_main_path, for_multiple_segments, ranks)
