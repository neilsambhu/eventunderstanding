...
def process_real_data_results(args):
    ...
    cmd = 'scp ' + input_path  + '/' + result_name + '.txt 131.247.2.235:~/'+remote_output_path+'/'
    print cmd
    os.system(cmd)

    # summarize performance and generate graph plots as images in png format
    cmd = 'ssh 131.247.2.235 "~/cumulative_ranked_performance.py '+remote_output_path+'/'+result_name+'.txt '+\
          remote_output_path+'/'+result_name+'_summary.txt '+\
          ' ' + metric + ' ' + ','.join(performance_indices) + '"'
    print cmd
    os.system(cmd)

    # transfer files created in the remote computer to the current computer
    cmd = 'scp 131.247.2.235:~/' + remote_output_path + '/' + result_name + '*.png ' + input_path  + '/'
    print cmd
    os.system(cmd)
    cmd = 'scp 131.247.2.235:~/' + remote_output_path + '/' + result_name + '_summary.txt ' + input_path  + '/'
    os.system(cmd)

    # delete images and files in the remote computer
    cmd = 'ssh 131.247.2.235 "rm -r ~/' + remote_output_path + '/' + result_name + '*.png"'
    os.system(cmd)
    cmd = 'ssh 131.247.2.235 "rm -r ~/' + remote_output_path + '/' + result_name + '*.txt"'
    os.system(cmd)

    # gather files that will be sent to the web repository where the results are kept for display on a web browser
    transfer_data = [input_path + '/' + experiment_name + '/' + result_name+'_summary.txt',
                     input_path + '/' + experiment_name + '/' + result_name+'_summary.png',
                     input_path + '/' + experiment_name + '/' + result_name+'_summary_per_video.png']
...
def process_results(args):
    ...
    remote_output_path = 'results/synthetic/' + subpath + '/' + experiment_name
    ...
    cmd = 'scp ' + input_path + '/' + folder_name+'.txt 131.247.2.235:~/'+remote_output_path+'/'
    print cmd
    os.system(cmd)
...
#    os.system(cmd)
    ...
    cmd = 'ssh 131.247.2.235 "~/cumulative_ranked_performance.py '+remote_output_path+'/'+folder_name+'.txt '+\
          remote_output_path+'/'+folder_name+'_summary.txt '+\
          ' ' + metric + ' ' + ','.join(performance_indices) + '"'
    print cmd
    os.system(cmd)

    cmd = 'scp 131.247.2.235:~/'+remote_output_path+'/'+folder_name+'*.png ' + input_path + '/'
    print cmd
    os.system(cmd)

    cmd = 'scp 131.247.2.235:~/'+remote_output_path+'/'+folder_name+'_summary.txt ' + input_path + '/'
    os.system(cmd)

    cmd = 'ssh 131.247.2.235 "rm -r ~/'+remote_output_path+'/'+folder_name+'*.png"'
    os.system(cmd)

    cmd = 'ssh 131.247.2.235 "rm -r ~/'+remote_output_path+'/'+folder_name+'*.txt"'
    os.system(cmd)
