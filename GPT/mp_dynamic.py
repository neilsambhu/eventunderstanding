...
def main():
    ...
    test_files_path = 'multiple_shot_test/1shots' #'test/3shots'
    experiment_name = '1shots_temporalbonds'
    ...
    # execute experiments
    run(experiment_name, test_files_path, number_of_iterations, dynamic_inference, with_temporal_bonds,
        number_of_processes, experiment_type, [[3,5,1],[10,30,10]], top_k)
...
##### READ PARAMETERS
    ...
    dynamic_inference = args[11]
    with_temporal_bonds = args[12]
...
#            os.system('read -p "pause"')
            ...
            (bests_configs,bests_energies,bests_accuracies,sorted_top_k_configs) = \
                    ps.mcmc_simulated_annealing_inference(videoname, number_of_iterations, global_jump_config,
                                                          video_results_path, dynamic_inference, with_temporal_bonds, top_k)
...
def run(experiment_name, test_files_path, number_of_iterations, dynamic_inference=False, with_temporal_bonds=False,
        number_of_processes='4', experiment_type='regular', synthetic_experiment_params=[[],[]], top_k=3):
    random.seed(12345)
