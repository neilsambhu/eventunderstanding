#!/home/students/fillipe/anaconda2/bin/python

from Ontology import *
from LibSVMPredictor import *
from ArtificialPredictor import *
from PatternSpace import *
from PCAPredictor import *
from LibSVMPredictorG import *
...
def main():
    ...
    if len(sys.argv) < 7:
        print './pattern_theory.py test_files_path experiment_name output_path synthetic|regular bond_weight_filename feature_thresh [OPTIONS]'
        print 'OPTIONS:'
        print '-rs <integer num> - randomization seed to differenciate one experiment from another'
        print '-ni <integer num> - number of iterations'
        ...
        print 'Note: the param feature_thresh is currently unused by the framework.'
        exit(1)
...
#    experiment_type = 'regular'
        ...
        if sys.argv[i] == '-tb':
            ...
            if i+1 < len(sys.argv):
                try: 
                    ...
                    rseed = 12345
                    print 'ERROR while attempting to set randomization seed. Randomization seed was set to 12345 since no number was passed!'
            ...
            if i+1 < len(sys.argv):
                try: 
                    ...
                    ni = 3000
                    print 'ERROR while attempting to set ni. ni was set to 3000 since no number was passed!'
                    print 'This might be an error in setting the input to the program. Please, revise it!'
                    exit(1)
            ...
            if i+1 < len(sys.argv):
                try: 
                    ...
                    top_k = 1
                    print 'ERROR while attempting to set top_k. top_k was set to 1 since no number was passed!'
                    print 'This might be an error in setting the input to the program. Please, revise it!'
                    exit(1)
    ...
    run(output_path, experiment_name, test_files_path, bond_weight_filename, number_of_iterations, feature_thresh, dynamic_inference, with_temporal_bonds,
        experiment_type, number_of_processes, top_k, [[2, 2, 1], [70, 70, 10]], run_seed)
...
def real_data_experiment(args):
    ...
    for i in range(number_of_trials):
        ...
        # create global jump configuration
        #print 'Create global jump for test case: ', infile, '\n';
        global_jump_config = ps.create_global_jump_configuration()
        ...
        for r in range(1,top_k+1):
            baseline_config = ps.get_baseline_configuration(global_jump_config, r);
            baseline_config_filename = video_results_path + '/baseline_rank' + str(r) + '_' + videoname + '.txt';
            ps.save_configuration(baseline_config_filename, baseline_config)
...
def run(output_path, experiment_name, test_files_path, bond_weights_filename, number_of_iterations, feature_thresh,
        ...
        top_k=3, synthetic_experiment_params=[[],[]], run_seed=12345):
    ...
    #generator_space_filename = 'cmu_generator_space.txt';
    generator_space_filename = 'generator_space.txt';    

    priors_config_filename = 'cmukitchen_priors.txt';
    #priors_config_filename = 'breakfast_priors.txt';
    #priors_config_filename = 'conceptnet_priors.txt'; 
    #priors_config_filename = 'conceptnet_priors_scale10000.txt';

    #modalities_config_filename = 'breakfast_modalities.txt';
    modalities_config_filename = 'modalities.txt';
...
#    pred = LibSVMPredictor('SVM', 'hof', 'breakfast_models_s1.txt', 'breakfast_labels.txt', './libsvm-3.17')
    pred = {};
    pred['cnn'] = LibSVMPredictorG('SVM', 'cnn', 'cmu_object-models.txt', 'cmu_object-labels.txt', './libsvm-3.17');
    pred['cnnflow'] = LibSVMPredictorG('SVM', 'cnnflow', 'cmu_event-models.txt', 'cmu_event-labels.txt', './libsvm-3.17');
    pred['spect400'] = LibSVMPredictorG('SVM', 'spect400', 'cmu_event-models.txt', 'cmu_event-labels.txt', './libsvm-3.17');
    pred['hog'] = LibSVMPredictorG('SVM', 'hog', 'cmu_object-models.txt', 'cmu_object-labels.txt', './libsvm-3.17');
    pred['hof'] = LibSVMPredictorG('SVM', 'hof', 'cmu_event-models.txt', 'cmu_event-labels.txt', './libsvm-3.17');
    pred['spe'] = PCAPredictor('PCA', 'spe', 1, '/home/students/fillipe/ICPR2016/NewModelFeatures');

    # set up ontology information
    ont = Ontology(generator_space_filename, modalities_config_filename, priors_config_filename);
