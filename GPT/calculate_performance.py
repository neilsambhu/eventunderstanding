...
def process_real_data_experiments(args):
    ...
    remote_output_path = 'results/real/' + dataset_name + '/' + subpath
    cmd = 'ssh 131.247.2.235 "mkdir -p ~/' + remote_output_path + '"'
    os.system(cmd)
    ...
    for i in range(len(inference_versions)):
        for j in range(len(experiments)):
            ...
            cmd = 'scp ' + input_path + '/' + filename + ' 131.247.2.235:~/' + remote_output_path + '/'
            os.system(cmd)

    cmd = 'ssh 131.247.2.235 "~/summarize_all.py real '+','.join(experiments)+' '+','.join(inference_versions) + \
          ' ' + str(rank_index) + ' ~/' + remote_output_path + ' ~/' + remote_output_path + ' ' + dataset_name + '"'
    print cmd
    os.system(cmd)
    ...
    for file in transfer_data:
        cmd = 'scp 131.247.2.235:~/' + remote_output_path + '/' + file + ' ' + input_path + '/'
        os.system(cmd)
...
def process_comparison_real_data_results(args):
    ...
    remote_output_path = 'results/real/' + dataset_name + '/' + subpath + '/' + experiment_name
    cmd = 'ssh 131.247.2.235 "mkdir -p ~/' + remote_output_path + '"'
    os.system(cmd)
    ...
    cmd = 'ssh 131.247.2.235 "~/compare_approaches_varying_rank.py real '+experiment_name+' '+','.join(inference_versions) + \
           ' ' + ','.join(ranks) + ' ~/' + remote_output_path + ' ~/' + remote_output_path + ' ' + dataset_name + '"'
    print cmd
    os.system(cmd)
...
def process_real_data_results(args):
    ...
    metric = args[5]
    subpath = args[6]
    generate_configuration_images = args[7]

    result_name = dataset_name + '_' + experiment_name + '_' + inference_version + '_performance'

    # create
    cmd = 'ssh c4lab06.csee.usf.edu "mkdir -p ~/public_html/results/real/' + dataset_name + '/' + subpath + '/' + experiment_name + '"'
    print cmd
    os.system(cmd)

    # summarize performance and generate graph plots as images in png format
    cmd = './cumulative_ranked_performance.py '+input_path+'/'+result_name+'.txt '+ \
          input_path+'/'+result_name+'_summary.txt '+' ' + metric + ' ' + ','.join(performance_indices) 
    print cmd
    os.system(cmd)

    # gather files that will be sent to the web repository where the results are kept for display on a web browser
    transfer_data = [input_path + '/' + result_name+'_summary.txt',
                     input_path + '/' + result_name+'_summary.png',
    ...
    if generate_configuration_images:
        ...
        transfer_data = [input_path + '/' + result_name + '_images',
                         input_path + '/' + result_name + '.html', 'style.css']
...
def process_results(args):
    ...
    metric = args[6]
    subpath = args[7]
    generate_configuration_images = args[8]
    ...
    cmd = 'ssh c4lab06.csee.usf.edu \"mkdir -p ~/public_html/results/synthetic/' + subpath + '/' + experiment_name+'\"'
    print cmd
    os.system(cmd)

    cmd = './cumulative_ranked_performance.py '+input_path+'/'+folder_name+'.txt '+input_path+'/'+folder_name+'_summary.txt ' \
          + ' ' + metric + ' ' + ','.join(performance_indices)
    print cmd
    os.system(cmd)

    # call matlab script to generate the images here...
    # matlab script will receive the inputfilename and outputfilename
    ...
    if generate_configuration_images == True:
        ...
        cmd = './create_results_page.py ' + input_path + '/' + folder_name + ' ' + input_path + '/' + \
              folder_name + '_images ' + input_path + '/' + folder_name + ' > out '
        os.system(cmd)

        # transfer data to remote machine to be seen on the website
...
def main():
    ...
    if len(sys.argv) < 6:
        print 'usage: ./organize_results.py input_path exp_type=real|synthetic experiment_name task=all|results|comparison subpath [images=-gci] [dataset_name]]'
        ...
        print 'ex.: ./calculate_performance.py youcook_synthetic_experiments synthetic 2shots results youcook -gci'
        exit(1)
            ...
            if experiment_type == 'synthetic':
                for d in range(degradation_range[0], degradation_range[1], degradation_range[2]):
                    ...
                    for r in range(rank_range[0],rank_range[1],rank_range[2]):
                        ...
                        tasks.append((input_path , experiment_name, version, d, r, ranks,
                                      metric, subpath + '/' + str(d) + '/' + str(r), generator_config_images))
                        print repr(tasks[-1])

            else:
                tasks.append((input_path, dataset_name, experiment_name, version, ranks, metric, subpath, generator_config_images))
                print repr(tasks[-1])
