...
def PatternTheory(inputFile, semanticBondPath, topK, outFile):
			...
			for line in fl:
				...
				for l1 in l:
					...
					if(semBondName != "Similarity"):
						semBondDict_Concept[l2[0]] = float(l2[1]) * 100
					else:
						semBondDict_Concept[l2[0]] = float(l2[1])
	...
	PS = inf.Inference(localSwapSpace, globalSwapSpace, topK, globalProposalChance)
	topKConfig = PS.run_inference()
	outFile1 = "/home/saakur/Desktop/test_S1_HRC/output/debugConfig.txt"
	topVal = 1
	ftFile = open(outFile, 'w')
	ftFile.write("Top %s results:\n" %topK)
	ftFile.close()
	for key in sorted(topKConfig):
		# print "-------Top %s Description-------" % topVal
		# topKConfig[key].configStatus()
		topKConfig[key].printConfig(outFile, localSwapSpace.keys())
		topKConfig[key].debugConfig(outFile + "1")
		topVal += 1
...
def main():
	...
	inputPath = "./test_S1_HRC/"

	outputPath = "./test_S1_HRC/output/"

	filelist = [ f for f in os.listdir(inputPath)]
	semanticBondPath = "./HRC_Priors/Contextual_Priors_V2/"
	topK = 10
