...
class PatternSpace:
    ...
    def __init__(self, ont, pred, features_filename, gt_filename, bond_weights_filename=None, strict_mode=True):
        ...
        self.__global_proposal_chance = 0.05
    ...
    def __not_in_the_list(self, config, best_config_list):
        ...
        for j in range(len(best_config_list)):
            if best_config_list[j].is_equal(config) == True:
                any_equal = False
                break
        return any_equal
    ...
    def mcmc_simulated_annealing_inference(self, input_name, number_of_iterations, global_jump_config, output_path,
                                           feature_support_thresh=.5, dynamic_inference=False, with_temporal_bonds=False, top_k=3, c_i=None):
        ...
        for i in range(N):
            ...
            if not dynamic_inference:
                if move_chance < self.__global_proposal_chance:
                    ...
                    new_c = self.global_proposal.propose(global_jump_config,with_temporal_bonds,feature_support_thresh)
                    #new_c.print_info()
                else:
...
#            print 'current config\'s energy: ',current_energy,' new config\'s energy: ',new_energy
            ...
            log_p = -(new_energy-current_energy)/T
            ...
            log_z = math.log(random.uniform(0,1),math.exp(1))

            # Accept the new configuration if the condition is satisfied
            # print 'log(p)=' + repr(log_p) + ' > log(z)=' + repr(log_z) + '?'

            if log_p > log_z: # print 'ACCEPTED' # For the cases when the local proposal returns the same configuration -- when it fails to suggest a new one
                if current_c != new_c:
...
#                current_c.print_info()
                ...
                if len(best_config_list) < top_k:
                    any_equal = False
                    for j in range(len(best_config_list)):
                        if best_config_list[j].is_equal(current_c) == True:
                            any_equal = True
                            break
                ...
                elif new_energy < best_energy_list[worst_best_config_index] and self.__not_in_the_list(current_c, best_config_list): # and new_energy not in best_energy_list:
...
#       best_profile_file.close()
    ...
    def get_baseline_configuration(self,global_jump_config):
        with_temporal_bonds = False
        new_c = self.global_proposal.propose(global_jump_config,with_temporal_bonds,'baseline')
        #accuracy_score = self.get_accuracy_score(new_c)
        #print 'BEST CONFIG ACCURACY SCORE: '+repr(accuracy_score)
        #os.system("read -p 'pause'")
        return new_c
    ...
    def create_global_jump_configuration(self):
        ...
        for feature in self.__features:
            ...
            if len(box) == 6: g_f.set_location(box[0],box[1],box[2],box[3],box[4],box[5])
